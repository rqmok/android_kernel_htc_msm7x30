...
#define PANEL_VIVO_SHARP		1
#define PANEL_VIVO_SONY		2
#define PANEL_VIVO_SHARP_CUT2		(PANEL_VIVO_SHARP | DRIVER_IC_CUT2)
#define PANEL_VIVO_SONY_CUT2		(PANEL_VIVO_SONY | DRIVER_IC_CUT2)
#define	PANEL_VIVO_HITACHI		PANEL_ID_VIVOW_HITACHI
...
#define VIVO_BR_DEF_HITACHI_PANEL_PWM		146
#define VIVO_BR_MIN_HITACHI_PANEL_PWM		10
#define VIVO_BR_MAX_HITACHI_PANEL_PWM		255
...
struct mdp_reg vivo_mdp_init_color[] = {
...
void vivo_mdp_color_enhancement(struct mdp_device *mdp_dev)
{
	struct mdp_info *mdp = container_of(mdp_dev, struct mdp_info, mdp_dev);
	mdp->write_regs(mdp, vivo_mdp_init_color, ARRAY_SIZE(vivo_mdp_init_color));
}
...
static int vivo_shrink_pwm(int brightness)
{
	...
	if(panel_type == PANEL_VIVO_HITACHI) {
		min_pwm = VIVO_BR_MIN_HITACHI_PANEL_PWM;
		def_pwm = VIVO_BR_DEF_HITACHI_PANEL_PWM;
		max_pwm = VIVO_BR_MAX_HITACHI_PANEL_PWM;
		     ...
		     struct msm_mddi_client_data *client_data)
{
	...
	if(panel_type == PANEL_VIVO_HITACHI) {
		...
		if (panel_type == PANEL_VIVO_SONY
			|| panel_type == PANEL_VIVO_SONY_CUT2) {
			...
			struct msm_mddi_client_data *client_data)
{
	...
	if(panel_type == PANEL_VIVO_HITACHI) {
		if (color_enhancement == 0) {
			vivo_mdp_color_enhancement(mdp_pdata.mdp_dev);
			color_enhancement = 1;
		}
...
int __init vivo_init_panel(void)
{
	...
	if (panel_type & DRIVER_IC_CUT2 || panel_type == PANEL_VIVO_HITACHI)
	...
	if(panel_type == PANEL_VIVO_HITACHI)
		vivo_backlight_driver.driver.name = "renesas_backlight";
