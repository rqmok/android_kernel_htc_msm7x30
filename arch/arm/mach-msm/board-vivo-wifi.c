...
static struct cntry_locales_custom msm7x30_wifi_translate_custom_table[] = {
...
static void *msm7x30_wifi_get_country_code(char *ccode)
{
	...
	size = ARRAY_SIZE(msm7x30_wifi_translate_custom_table);
	...
	for (i = 0; i < size; i++) {
		if (!strcmp(
			ccode, msm7x30_wifi_translate_custom_table[i].iso_abbrev))
